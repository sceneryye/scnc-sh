...
module ApplicationHelper
  ...
  def flash_class(type)
	...
	def truncate2(content,options={})
      ...
      def page_title
        (content_for :title).present? ? (content_for :title) : '上海儿童营养中心'
      end

      def meta_keywords
        (content_for :keywords).present? ? (content_for :keywords) : '上海儿童营养中心'
      end

      def meta_description
        (content_for :description).present? ? (content_for :description) : '上海儿童营养中心'
      end
